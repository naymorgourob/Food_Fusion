...
// Every destination here is a page Staff can genuinely open — no item
// links anywhere that would 403. Inventory and Customers are absent on
// purpose: GET /inventory is authorizeAdmin (zero Staff access, not even
// read), and Customers is likewise Admin-only, so listing either would be
// a dead link dressed as a feature.
const NAV_ITEMS = [
  { label: 'Dashboard', to: ROUTES.STAFF_HOME, icon: LayoutDashboard, end: true },
  { label: 'Kitchen Queue', to: ROUTES.STAFF_KITCHEN, icon: ChefHat },
  { label: 'Orders', to: ROUTES.STAFF_ORDERS, icon: ClipboardList },
  { label: 'Reservations', to: ROUTES.STAFF_RESERVATIONS, icon: CalendarCheck },
  { label: 'Tables', to: ROUTES.STAFF_TABLES, icon: LayoutGrid },
  { label: 'Profile', to: ROUTES.STAFF_PROFILE, icon: UserRound },
]

/**
 * Staff's own rail (UI-07) — deliberately a separate component from the
 * Admin Sidebar, not a redesign of it. Sidebar.jsx is shared chrome that
 * Admin also renders inside DashboardLayout; editing it in place would
 * have changed how Admin's dashboard looks, which this task doesn't
 * authorize touching. Staff instead gets its own layout branch (see
 * StaffLayout.jsx and App.jsx), styled like the rest of the redesigned
 * customer-facing app but with a workspace vocabulary instead of a
 * dining one.
 */
export function StaffSidebar({ collapsed, onToggleCollapsed, mobileOpen, onCloseMobile }) {
    ...
    <>
      ...
      >
        <div className="flex h-16 flex-none items-center justify-between px-4">
          <NavLink to={ROUTES.STAFF_HOME} className="flex items-center gap-2.5 overflow-hidden">
        ...
        <div className="flex-none border-t border-white/10 p-3">
          <div className={`mb-2 flex items-center gap-3 px-2 py-1.5 ${collapsed ? 'lg:justify-center lg:px-0' : ''}`}>
            ...
            <span className={`flex min-w-0 flex-col leading-tight ${collapsed ? 'lg:hidden' : ''}`}>
              ...
              <span className="truncate text-xs text-white/50">{user?.position ?? 'Staff'}</span>
            </span>
          </div>

          <NavLink
            to={ROUTES.STAFF_SETTINGS}
            onClick={onCloseMobile}

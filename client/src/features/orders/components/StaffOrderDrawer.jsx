...
// never offered a transition the server would reject.
...
/**
 * Full order detail + every staff action, as a slide-in drawer (UI-07).
 *
 * This is Staff's own presentation of the exact same data and handlers
 * OrderDetailsModal (the Admin/shared component) already uses — status
 * change, staff assignment, estimated time — none of it rewritten.
 * OrderDetailsModal itself is left alone: it's built on Modal.jsx, which
 * every Admin CRUD form in the app also uses, so restyling it would have
 * changed how Admin's whole panel looks.
 */
export function StaffOrderDrawer({
  order,
  isOpen,
  onClose,
  onStatusChange,
  ...
  // key={order.id} whenever a different order is opened (see
  // StaffOrdersPage), so these only ever need to be set once per mount —
  // exactly the pattern OrderDetailsModal already uses for the same forms.
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="flex flex-col gap-6">
                ...
                {/* --- Status --------------------------------------- */}
                <section className="flex flex-col gap-2">
                  <label htmlFor="staff-order-status" className="text-xs font-semibold tracking-wide text-body-faint uppercase">
                    Update status
                  </label>
                  <select
                ...
                {/* --- Assign staff ----------------------------------- */}
                <section className="flex flex-col gap-2">
                  <label htmlFor="staff-order-assign" className="text-xs font-semibold tracking-wide text-body-faint uppercase">
                    Assign staff
                  </label>
                  <select
                    id="staff-order-assign"
